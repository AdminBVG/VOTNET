  ...
  <div class="page">
    ...
    <mat-card *ngIf="editMode() && canClose && (!quorum() || (quorum()?.present||0) === 0)" class="mat-elevation-z1">
      ...
      <form [formGroup]="editForm" (ngSubmit)="saveEdit()" class="edit-grid">
        ...
        <mat-form-field appearance="outline" *ngIf="canClose">
          <mat-label>Perfil de firma</mat-label>
          <mat-select [value]="(electionInfo()?.signingProfile || electionInfo()?.SigningProfile) ?? ''" (selectionChange)="setSigningProfile($event.value)" aria-label="Seleccionar perfil de firma">
            <mat-option [value]="">Sin perfil</mat-option>
            <mat-option *ngFor="let p of signingProfiles" [value]="p.alias">{{p.alias}} — {{p.subject || p.Subject}}</mat-option>
          </mat-select>
        </mat-form-field>
        <mat-slide-toggle [checked]="signingRequired" (change)="toggleSigning($event.checked)" *ngIf="canClose">Requerir firma para certificar</mat-slide-toggle>
        <div class="actions">
...
export class ElectionDetailComponent implements AfterViewInit {
  ...
  setSigningProfile(alias: string){ this.http.put(`/api/elections/${this.id()}`, { signingProfile: alias }).subscribe({ next: _=> { this.snack.open('Perfil de firma actualizado','OK',{duration:1500}); this.loadElectionInfo(); }, error: _=> this.snack.open('No se pudo actualizar perfil','OK',{duration:2000}) }); }
