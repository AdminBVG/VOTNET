  ...
  <div class="p-4">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-card p-4">
      <form [formGroup]="form" (ngSubmit)="save()" class="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-3 items-start">
        ...
        <div>
          <label class="field-label">Storage Root</label>
        ...
        <div><label class="field-label">Host</label><input uiInput formControlName="host"></div>
        <div><label class="field-label">Puerto</label><input uiInput type="number" formControlName="port"></div>
        <div><label class="field-label">Usuario</label><input uiInput formControlName="user"></div>
        <div><label class="field-label">Remitente (From)</label><input uiInput formControlName="from"></div>
        <div class="col-span-full h-px bg-gray-200 my-1"></div>
        <h3 class="col-span-full mt-2 text-base font-semibold">Azure AD</h3>
        <div><label class="field-label">Tenant ID</label><input uiInput formControlName="tenantId"></div>
        <div><label class="field-label">Client ID</label><input uiInput formControlName="clientId"></div>
        <div><label class="field-label">Client Secret</label><input uiInput formControlName="clientSecret"></div>
        <div class="col-span-full h-px bg-gray-200 my-1"></div>
        <h3 class="col-span-full mt-2 text-base font-semibold">Branding</h3>
        <div class="col-span-full"><label class="field-label">Logo URL</label><input uiInput formControlName="logoUrl"></div>
        ...
        <div class="col-span-full">
          ...
          <ng-container *ngIf="!cspAdvanced; else advancedCspTpl">
            <label class="field-label">Content-Security-Policy</label>
          ...
          <ng-template #advancedCspTpl>
            <div class="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-3">
              <div><label class="field-label">default-src</label><input uiInput [ngModel]="cspForm.default" (ngModelChange)="setCsp('default',$event)" [ngModelOptions]="{standalone:true}" placeholder="'self'"></div>
              <div><label class="field-label">script-src</label><input uiInput [ngModel]="cspForm.script" (ngModelChange)="setCsp('script',$event)" [ngModelOptions]="{standalone:true}" placeholder="'self' 'unsafe-inline'"></div>
              <div><label class="field-label">style-src</label><input uiInput [ngModel]="cspForm.style" (ngModelChange)="setCsp('style',$event)" [ngModelOptions]="{standalone:true}" placeholder="'self' 'unsafe-inline'"></div>
              <div><label class="field-label">img-src</label><input uiInput [ngModel]="cspForm.img" (ngModelChange)="setCsp('img',$event)" [ngModelOptions]="{standalone:true}" placeholder="'self' data:"></div>
              <div><label class="field-label">connect-src</label><input uiInput [ngModel]="cspForm.connect" (ngModelChange)="setCsp('connect',$event)" [ngModelOptions]="{standalone:true}" placeholder="'self'"></div>
              <div><label class="field-label">frame-ancestors</label><input uiInput [ngModel]="cspForm.frames" (ngModelChange)="setCsp('frames',$event)" [ngModelOptions]="{standalone:true}" placeholder="'none'"></div>
            </div>
            <div class="mt-2"><label class="field-label">CSP generado</label><textarea uiInput rows="3" [value]="builtCsp" readonly></textarea></div>
          </ng-template>
        </div>
        <div class="col-span-full h-px bg-gray-200 my-1"></div>
        <h3 class="col-span-full mt-2 text-base font-semibold">Firmas</h3>
        <div class="col-span-full">
          <div class="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-3 items-end">
            <div><label class="field-label">PFX Password</label><input uiInput formControlName="pfxPassword" type="password"></div>
            <div><label class="field-label">Alias del perfil</label><input uiInput formControlName="signingAlias" placeholder="ej: Junta2025"></div>
            <div>
              <label class="field-label">Archivo PFX</label>
              <input type="file" (change)="onPfxInput($event)" accept=".pfx,.p12">
            </div>
            <div class="justify-self-end"><button uiBtn="secondary" type="button" (click)="uploadPfx()">Subir PFX</button></div>
          </div>
          <div class="text-sm opacity-70 mt-1">PFX actual: {{ pfxPath || 'Ninguno' }}</div>
        </div>
        <div class="col-span-full h-px bg-gray-200 my-1"></div>
        <h3 class="col-span-full mt-2 text-base font-semibold">Perfiles</h3>
        <table class="table-base table-compact thead-sticky row-zebra" *ngIf="profiles.length">
          <thead><tr><th class="text-left p-2">Alias</th><th class="text-left p-2">Subject</th><th class="w-40 p-2"></th></tr></thead>
          <tbody>
            <tr *ngFor="let p of profiles" class="border-t">
              <td class="p-2">{{p.alias}}</td>
              <td class="p-2">{{(p.subject || p.Subject) | slice:0:100}}</td>
              <td class="p-2 text-right"><button uiBtn="danger" size="sm" type="button" (click)="deleteProfile(p.alias)">Eliminar</button></td>
            </tr>
          </tbody>
        </table>
      </form>
...
export class AdminConfigComponent{
  ...
  onPfxInput(e: Event){ const input = e.target as HTMLInputElement | null; const file = input?.files && input.files.length ? input.files[0] : null; this.onPfx(file); }
